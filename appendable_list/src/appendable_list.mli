...
(** A polymorphic data structure parametrized by ['a] to represent lists of elements of
    ...
    The following operations all run in constant time: [empty], [of_list], [singleton],
    [append], [concat], [add_front], [add_back]
    ...
    The monad exported by the module is semantically the same as the one in List. That is:
    [bind t f] applies [f] to each element of [t] and append the resulting list respecting
    the order in which the elements appear in [t]. *)
