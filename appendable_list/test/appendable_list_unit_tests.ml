module%test [@name "individual test cases"] _ : module type of Appendable_list = struct
  open Core
...
module%test [@name "semantics"] _ : module type of Appendable_list = struct
  open Core
  open Appendable_list
  module Stable = Stable
  ...
  let%test_unit "sexp_of_t" = test_sexp_of_t ~sexp_of:[%sexp_of: For_testing.Element.t t]
    ...
    Quickcheck.test
      ...
      ~f:(fun ts ->
        [%test_eq: For_testing.Element.t list option]
          ...
          (Option.map
             ~f:to_list
             (List.max_elt ts ~compare:[%compare: For_testing.Element.t Stable.V1.t])))
  ;;
  ...
  let%test_unit "bin_io (Stable.V1)" =
    Quickcheck.test [%quickcheck.generator: int list] ~f:(fun xs ->
      [%test_eq: bytes]
        ...
        (Bin_prot.Writer.to_bytes [%bin_writer: int Stable.V1.t] (of_list xs)))
  ;;
