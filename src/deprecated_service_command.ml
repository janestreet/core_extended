...
let command ~lock_file ~name main =
  ...
  let base_cmd ~verb main =
    Deprecated_command.create (fun () -> exit (main ()))
  ...
  let stop_cmd ~verb main =
    Deprecated_command.create (fun stop_signal -> exit (main ~stop_signal))
      ~summary:(summary ~verb)
      ~usage_arg:"[-kill]"
      ~init:(Fn.const Signal.term)
      ~flags:[
        Deprecated_command.Flag.noarg_acc "-kill" (Fn.const Signal.kill)
          ~doc:" send SIGKILL instead of SIGTERM"
      ]
      ~final:(fun signal anons -> assert_no_anons anons; signal)
  in
  Deprecated_command.group ~summary:(summary ~verb:"manage") [
