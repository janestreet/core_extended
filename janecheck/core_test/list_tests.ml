...
TEST_MODULE "random" = struct
  ...
  module Make (T : T) (Math : Math) = struct
    ...
    module Q = Janecheck.Configure (struct
        include Janecheck
        let default_seed =
          `Deterministic
    ...
    TEST_UNIT "is_empty false" =
      Q.test ~sexp_of:<:sexp_of< t list >>
        (G.tuple t_gen (G.list t_gen) >>| fun (x,list) ->
         x::list)
        ~f:(fun non_empty ->
          <:test_result< bool >>
    ...
    TEST_UNIT "sum vs fold" =
      Q.test ~sexp_of:<:sexp_of< t list * (t -> Math.t) >>
        (G.tuple (G.list t_gen) (G.fn t_obs Math.t_gen))
        ~f:(fun (list, f) ->
          <:test_eq< Math.t >>
    ...
    TEST_UNIT "for_all vs exists" =
      Q.test ~sexp_of:<:sexp_of< t list * (t -> bool) >>
        (G.tuple (G.list t_gen) (G.fn t_obs G.bool))
        ~f:(fun (list, f) ->
          <:test_eq< bool >>
            (List.for_all list ~f)
    ...
    TEST_UNIT "max_elt vs min_elt" =
      Q.test ~sexp_of:<:sexp_of< t list * (t -> t -> int) >>
        (G.tuple (G.list t_gen) (G.compare_fn t_obs))
        ~f:(fun (list, cmp) ->
          <:test_eq< t option >>
    ...
    TEST_UNIT "map vs bind" =
      Q.test ~sexp_of:<:sexp_of< t list * (t -> t) >>
        (G.tuple (G.list t_gen) (G.fn t_obs t_gen))
        ~f:(fun (list, f) ->
          <:test_eq< t list >>
            (List.map list ~f)
    ...
    TEST_UNIT "monad left identity" =
      Q.test ~sexp_of:<:sexp_of< t * (t -> t list) >>
        (G.tuple t_gen (G.fn t_obs (G.list t_gen)))
        ~f:(fun (x, f) ->
          <:test_eq< t list >>
    ...
    TEST_UNIT "init + nth_exn" =
      Q.test ~sexp_of:<:sexp_of< int * int * (int -> t) >>
        (G.size >>= fun size ->
         G.tuple3
           (G.return size)
           (G.int_between ~lower_bound:(`Inclusive 0) ~upper_bound:(`Exclusive size))
           (G.fn O.int t_gen))
        ~f:(fun (size, i, f) ->
          <:test_result< t >>
    ...
    TEST_UNIT "rev_append vs rev + append" =
      Q.test ~sexp_of:<:sexp_of< t list * t list >>
        (G.tuple (G.list t_gen) (G.list t_gen))
        ~f:(fun (list1, list2) ->
          <:test_eq< t list >>
    ...
    TEST_UNIT "unzip + iter2_exn vs iter" =
      Q.test ~sexp_of:<:sexp_of< (t * t) list * (t -> t -> t) >>
        (G.tuple (G.list (G.tuple t_gen t_gen)) (G.fn2 t_obs t_obs t_gen))
        ~f:(fun (pair_list, f) ->
          <:test_eq< t list >>
            (let q = Queue.create () in
    ...
    TEST_UNIT "unzip + fold2_exn + fold" =
      Q.test ~sexp_of:<:sexp_of< (t * t) list * t * (t -> t -> t -> t) >>
        (G.tuple3 (G.list (G.tuple t_gen t_gen)) t_gen (G.fn3 t_obs t_obs t_obs t_gen))
    ...
    TEST_UNIT "unzip + for_all2_exn vs for_all" =
      Q.test ~sexp_of:<:sexp_of< (t * t) list * (t -> t -> bool) >>
        (G.tuple (G.list (G.tuple t_gen t_gen)) (G.fn2 t_obs t_obs G.bool))
        ~f:(fun (pair_list, f) ->
          <:test_eq< bool >>
            (let list1, list2 = List.unzip pair_list in
    ...
    TEST_UNIT "partition_map vs filter_map" =
      ...
      Q.test ~sexp_of:<:sexp_of< t list * (t -> [ `Fst of t | `Snd of t ]) >>
        (G.tuple (G.list t_gen)
           (G.fn t_obs (G.variant t_gen t_gen >>| partition_of_variant)))
        ~f:(fun (list, f) ->
    ...
    TEST_UNIT "concat_mapi vs concat_map" =
      Q.test ~sexp_of:<:sexp_of< t list * (t -> t list) >>
        (G.tuple (G.list t_gen) (G.fn t_obs (G.list t_gen)))
        ~f:(fun (list, f) ->
          <:test_eq< t list >>
    ...
    TEST_UNIT "unzip + map3_exn vs map" =
      Q.test ~sexp_of:<:sexp_of< (t * (t * t)) list * (t -> t -> t -> t) >>
        (G.tuple
           (G.list (G.tuple t_gen (G.tuple t_gen t_gen)))
