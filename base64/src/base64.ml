...
module Make(D : sig
  ...
  let encode_char i =
    ...
    else failwithf "Impossible Base64 encoder bug: \
                    not representable as a base64 character: %d" i ()
  ;;
  ...
  let decode_char c =
    if 'A'<=c && c<='Z' then Data (Char.to_int c - Char.to_int 'A')
    else if 'a'<=c && c<='z' then Data (Char.to_int c - Char.to_int 'a' + 26)
    else if '0'<=c && c<='9' then Data (Char.to_int c - Char.to_int '0' + 52)
    else if c=D.char62 then Data 62
    else if c=D.char63 then Data 63
    else if D.ignore_char c then Skip
    else Other
  ;;
  ...
  let read_base64_block source i =
    let rec read_char i =
      if i >= String.length source then
        ...
        match String.get source i |> decode_char with
        | Data b -> Some (b, i+1)
        | Skip -> read_char (i+1)
        | Other -> None)
    in
  ...
  let decode source =
    ...
    in
    let finish i j =
      let result = String.sub dest ~pos:0 ~len:j in
      let rec stop i =
        if i < String.length source then (
          let c = String.get source i in
          if D.ignore_char c || c = D.pad_char then
            stop (i+1)
          else
            Some (`Unconsumed_data (String.sub source ~pos:i ~len:(String.length source - i))))
        else
          None
      in
  ...
  let decode_exn source =
    ...
    | decoded, Some (`Unconsumed_data unconsumed_data) ->
      raise_s
        [%message "[decode_exn] failed with warnings"
          (decoded : string)
